...
resource "aws_iam_policy" "iam_policy_secretsmanager" {
  name = lower("${var.App_Name}-${var.Name}-${var.Env_Type}-secretsmanager")
...
resource "aws_iam_policy" "iam_policy_ecr" {
  name = lower("${var.App_Name}-${var.Name}-${var.Env_Type}-ecr")
  #   role = aws_iam_role.ec2_iam_role.id
...
resource "aws_iam_user" "jenkins-iam-user" {
  name = lower("${var.App_Name}-${var.Name}-${var.Env_Type}-jenkins")
}
